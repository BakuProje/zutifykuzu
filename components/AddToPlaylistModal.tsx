...
import { X, Plus, Music, Heart, CheckCircle2 } from 'lucide-react';
...
export function AddToPlaylistModal() {
  ...
  const handleAddToPlaylist = async (playlist: Playlist) => {
    ...
    if (playlist.tracks.some(t => t.videoId === trackToAdd.videoId)) {
      showSuccessState(`Lagu sudah ada di ${playlist.name}`);
      return;
    }
    ...
    showSuccessState(`Berhasil ditambahkan ke ${playlist.name}`);
  };

  const handleToggleLike = async () => {
    if (!trackToAdd) return;
    const isLiked = await db.isLiked(trackToAdd.videoId);
    if (isLiked) {
      await db.removeLikedSong(trackToAdd.videoId);
      showSuccessState('Dihapus dari Lagu Disukai');
    } else {
      await db.addLikedSong(trackToAdd);
      showSuccessState('Ditambahkan ke Lagu Disukai');
    }
  };
  ...
  const handleCreateAndAdd = async () => {
    ...
    setNewPlaylistName('');
    setIsCreating(false);
    showSuccessState(`Berhasil membuat playlist ${newPlaylistName}`);
  };
        ...
        <div className="flex-1 overflow-y-auto p-2">
          {showSuccess ? (
            <div className="flex flex-col items-center justify-center py-12 animate-in zoom-in-95 duration-300">
              <div className="w-20 h-20 bg-[#FA243C]/10 rounded-full flex items-center justify-center mb-6">
                <CheckCircle2 className="w-10 h-10 text-[#FA243C]" />
              </div>
            ...
            <>
              ...
              >
                <div className="w-12 h-12 bg-white/10 rounded-md flex items-center justify-center group-hover:bg-[#FA243C]/20 transition-colors">
                  <Heart className="w-6 h-6 text-[#FA243C] fill-[#FA243C]/20" />
                </div>
                <span className="text-white font-medium">Lagu Disukai</span>
              </button>
