    ...
    <main className="min-h-screen pb-32">
      ...
      <div className="relative h-[40vh] min-h-[300px] w-full">
        ...
        <div className="absolute top-0 left-0 right-0 p-4 flex justify-between items-center z-10">
          <button onClick={() => router.back()} className="p-2 bg-black/20 backdrop-blur-md rounded-full text-white hover:bg-black/40 transition shrink-0">
            <ArrowLeft className="w-6 h-6" />
          </button>
          
          <DesktopSearch />

          <button className="p-2 bg-black/20 backdrop-blur-md rounded-full text-white hover:bg-black/40 transition shrink-0">
              ...
              {artist.topAlbums.map((album: any, index: number) => (
                ...
                >
                  <div className="relative aspect-square rounded-lg overflow-hidden mb-3">
                    <ImageWithFallback 
                      src={getHighResImage(album.thumbnails?.[album.thumbnails.length - 1]?.url, 400)} 
                      alt={album.name} 
                      fill 
            ...
            <div className="flex overflow-x-auto no-scrollbar gap-4 snap-x snap-mandatory pb-4">
              {artist.topSingles.map((single: any, index: number) => (
                ...
                >
                  <div className="relative aspect-square rounded-lg overflow-hidden mb-3">
                    <ImageWithFallback 
                      src={getHighResImage(single.thumbnails?.[single.thumbnails.length - 1]?.url, 400)} 
                      alt={single.name} 
                      fill 
                      className="object-cover group-hover:scale-105 transition-transform duration-300"
                    />
              ...
              {artist.topVideos.map((video: any, index: number) => (
                ...
                >
                  <div className="relative aspect-video rounded-lg overflow-hidden mb-3">
                    <ImageWithFallback 
                      src={getHighResImage(video.thumbnails?.[video.thumbnails.length - 1]?.url, 400)} 
                      alt={video.name} 
                      fill 
                      className="object-cover group-hover:scale-105 transition-transform duration-300"
                    />
            ...
            <div className="flex overflow-x-auto no-scrollbar gap-4 snap-x snap-mandatory pb-4">
              {artist.featuredOn.map((playlist: any, index: number) => (
                ...
                >
                  <div className="relative aspect-square rounded-lg overflow-hidden mb-3">
                    <ImageWithFallback 
                      src={getHighResImage(playlist.thumbnails?.[playlist.thumbnails.length - 1]?.url, 400)} 
                      alt={playlist.name} 
                      fill 
                      className="object-cover group-hover:scale-105 transition-transform duration-300"
                    />
