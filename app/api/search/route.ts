...
import { enrichTracks } from '@/lib/server/metadata';
...
export async function GET(request: Request) {
  ...
  try {
    ...
    if (type === 'all') {
      ...
      // Separate tracks (songs/videos) to enrich them
      const tracks = results.filter((r: any) => r.type === 'SONG' || r.type === 'VIDEO');
    ...
    // If no type is specified, search sequentially and prioritize songs
    ...
    await enrichTracks(results);
    
    // Prioritize songs, then videos, then artists
    const finalResults = [...results, ...artists];
